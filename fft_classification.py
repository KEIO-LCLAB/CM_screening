...
FEATURE_Num = 50 ## the number of features
...
parameter = "sample" ### set arbitrary name
# From, To = 4, 10 ## select feature, if you want to use only partial feature
...
if __name__ == '__main__':
    ...
    for type in range(len(file_list)): ## "type==0" means health data, and "type==1" means patient data
        ...
        for file in file_list[type]:
            ...
            data[data_num] = data_origin[100:100+N_Origin, :] if len(data_origin)>(100+N_Origin) else data_origin[len(data_origin) - N_Origin:len(data_origin), :]
            if np.isnan(data[data_num]).any(): ## check NAN
                print(file)
                data[data_num] = pd.DataFrame(data[data_num]).fillna(method='ffill').to_numpy() ## if NAN exists, it is filled by the previous one
            data_num += 1
            if data_num == HANDDATA_Num: ## process every four data(L1, L2, R1, R2)
                ...
                for i in range(int(N_Origin/N)): # 256 frames are divided by four, and the process is done every 64 frames
                    for loop in range(HANDDATA_Num): ## four times (L1,L2,R1,R2)
                        ...
                        for sensor_num in range(FEATURE_Num):
                            ...
                            x.append(minmax_scale(F_window)[:int(N/2)])  ## MinMax Normalization & get the low frequency part
